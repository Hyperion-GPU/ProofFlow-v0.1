...
> **Status: PARTIALLY CLOSED on 2026-05-20** — VSCode screenshots delivered on branch
> `polish/v0-1-x-dogfood-postclosure-audit-refresh` from working-tree commit base
> `40decccc37164bd58a1fd4f054789272218ca9e9` (uncommitted on this branch). The
> Demo_Asset GIF (originally folded in via the scope-expansion note below)
> remains deferred under this same anchor.
>
> Delivered evidence:
> - `vscode-proofflow/docs/assets/proofflow-v0_1_x-vscode-inline-audit.png` — Case Detail webview, Evidence & Claims tab, evidence hover triggers claim highlight (cross-link interaction).
> - `vscode-proofflow/docs/assets/proofflow-v0_1_x-vscode-approve-gate.png` — Approve Gate webview rendering a `pending` LocalProof move_file action with From/To/Reason rows and Approve & Execute / Reject buttons.
>
> Run context: dogfood case `f2a53d2c-5184-4f0b-b988-5a3c097d400d` (`Dogfood v0.2 ledger UI`), full ledger lifecycle (start contract → algorithm decision → cost budget → start snapshot → evidence → claim → evaluate → finish) + an Approve Gate `Approve & Execute` against a `move_file` action (one of the four `pending` rows generated by `localproof/suggest-actions` on `.playwright-mcp/`). Exported packet: `.proofflow/exports/f2a53d2c-2026-05-20T07-28-28-166Z.md`.
>
> Still deferred under this anchor: `docs/assets/proofflow-v0_1_x-vscode-flow.gif` (optional) and `docs/assets/proofflow-v0_1_x-dogfood-<channel>.gif` (Demo_Asset, task 7.2). Same privacy rules apply when those land.
