...
本里程碑严格遵守四条硬约束：(1) **不切新 release tag**——Existing_Release_Tag 9 集合 `{v0.1.0, v0.1.0-rc1, v0.1.3, v0.1.3.1, v0.1.4, v0.1.5, v0.1.6, v0.1.6.1, v0.1.8}` 在 milestone start / close / Pre-Merge Gate re-check 三次 capture 全程 byte-identical（`git ls-remote --tags origin` SHA-256 = `0B4AAEAC…329E`；`gh release list` SHA-256 = `CACAB81B…979F`）。(2) **main-only**——所有 Polish_PR 候选通过独立 `polish/<short-slug>` 分支提，从未直接 push `main`；当前已开 PR 仅 #121（DOC shape，OPEN）作 GHA channel evidence 触发载体，5 份 Quickstart_Doc_Set 修复仍在 working tree 待用户后续按 Backlog 锚点打包。(3) **不弱化 Trust_Baseline**——Policy Gate scope 仍严格 `{move_file, rename_file}`（task 2.2 `rc2_policy_gate_smoke.py` 14/14 Pass）；hash-guarded undo 与 sensitive-file omission 用例在 backend pytest 中 Pass（task 2.1 313 passed / 3 skipped）；live DB restore 仍 deferred（task 2.2 `backup_restore_api_smoke.py` 经 `_assert_isolated_paths` 守卫验证，post-run `backend\data\proofflow.db` mtime / size 与 baseline 完全一致）；3 条 smoke 全 Pass。(4) **不引入新产品功能**——本里程碑 0 OPPORTUNISTIC_FIX shape PR、0 net-new feature 关键词命中、0 Policy Gate action 类型扩展。

三渠道结果按 design §Schema 3 严格采集：**MCP_Channel = Pass**（Codex client + 5/6 tools `proofflow_scan` / `proofflow_suggest` / `proofflow_approve_execute` / `proofflow_decide` / `proofflow_export_packet` 实际触发，packet path = `docs/examples/proof_packet_v0_1_x_localproof_dogfood.md`）；**GHA_Channel = Pass**（PR #121 触发 `proofflow-pr-review.yml`，4 字段全在：`pr_url` / `run_url` / `artifact_id 7063049346` / `comment_url`，与 CI Status 表 `proofflow-pr-review.yml` Run URL 交叉一致）；**VSCode_Channel = Deferred**（`vscode-extension` commit SHA `a94d190ce03332c1c480e8d887be2ebf8df9daba`；本里程碑执行环境无可视 VS Code 窗口可抓取，按 design §Error Handling「把降级如实记录而不是粉饰为 Pass」原则挂 PLANS anchor `vscode-channel-screenshots-deferred-from-v0-1-x-dogfood`）。CI 6 个 workflow 在 Latest_Main 全部 `conclusion=success`（`backend.yml` / `frontend.yml` / `lint.yml` / `mcp-server.yml` push-on-main 直查；`proofflow-pr-review.yml` 复用 PR #121 Run；`publish-mcp.yml` 取 `mcp-v0.1.2` baseline）。

7 条 Findings（F-1 ~ F-7）按 Requirement 7.2 严格判定全部归类为 `Out_Of_Scope_Finding`：F-1 ~ F-5 是 5 份 Quickstart_Doc_Set polish（README.md / README.zh-CN.md / docs/ledger_quickstart_mcp.md / docs/codex_workflow.md / docs/V0_1_DOGFOOD.md，均已在 working tree 修复且 task 6.3 lint 0 stale，但因对应 polish/quickstart-* PR 仍未合并，按 Requirement 7.5 backlog honesty 暂归 out-of-scope）挂 anchor `quickstart-polish-prs-pending-packaging-from-v0-1-x-dogfood`；F-6 是 VSCode 截图与 Demo_Asset GIF 同根因 deferred（用户 Option E 决策合并 anchor）挂 `vscode-channel-screenshots-deferred-from-v0-1-x-dogfood`；F-7 是 AgentGuard public packet 缺席（用户 Option C 决策仅交付 LocalProof 一份）挂 `agentguard-public-packet-deferred-from-v0-1-x-dogfood`。7 行 8 字段全填，0 静默丢弃；待用户后续打包 5 个 polish/quickstart-* PR 合并后由后续里程碑 / task 9.2 把 F-1 ~ F-5 升级回 `In_Scope_Fix` 并把 `link` 替换为真实 merged PR URL。
...
- working tree（**未打包成 PR、未合并到 main**）：5 份 Quickstart_Doc_Set 的 polish 修复（task 6.2 在 README.md / README.zh-CN.md / docs/ledger_quickstart_mcp.md / docs/codex_workflow.md / docs/V0_1_DOGFOOD.md 用 `Push-Location` / `Pop-Location` 替代隐式 `cd`、固定后端端口 `8787`、entry 收敛到 `proofflow.main:app` + MCP 收敛到 `pip install proofflow-mcp`、含空格路径双引号包裹），等待用户按 PLANS anchor `quickstart-polish-prs-pending-packaging-from-v0-1-x-dogfood` 打包 5 个 `polish/quickstart-*` PR。
- 新增 `PLANS.md` 三段 Out_Of_Scope_Finding anchor（task 9.3）：`vscode-channel-screenshots-deferred-from-v0-1-x-dogfood` / `agentguard-public-packet-deferred-from-v0-1-x-dogfood` / `quickstart-polish-prs-pending-packaging-from-v0-1-x-dogfood`，与 Findings 表 7 行 `link` 列双向交叉一致。
...
- 5 个 polish/quickstart-* PR 未提（按用户 Option B 决策不打包成 PR、不 push）；working tree 修复仍在但不计入 In_Scope_Fix（Findings 表 7 行 0 In_Scope_Fix，避免占用 In_Scope_Fix 集合的「合并 PR URL」承诺）。
...
- Opportunistic Polish_PRs landed on `main` only（task 9.1 / 9.2 / 9.3 实采：本里程碑 In_Scope_Fix Polish_PR merged 集合 = {}，PR #121 OPEN 作 GHA channel evidence 触发载体；5 个 polish/quickstart-* working-tree-only 待用户打包；按 design §GHA Channel Trigger Strategy 与 Requirement 8.1）。
...
- Evidence:
  ...
  - `run_url`: <https://github.com/Hyperion-GPU/ProofFlow-v0.1/actions/runs/26045394262>
  - `artifact_id`: `7063049346`
...
- Notes: PR #121 是**纯文档 Polish_PR**（DOC shape，design §Schema 6），**非 In_Scope_Fix**——其用途显式声明为「专门为触发 GHA review 而提」，与 design §GHA Channel Trigger Strategy / Requirement 2.4 一致：当本里程碑当下无 In_Scope_Fix 可作 review 载体时，提一个纯文档微小 PR 触发 workflow，不构造假修复也不污染 In_Scope_Fix 集合。触发路径合规：PR 走 `pull_request` 事件触发 workflow，**未**在 `main` 上直接 push、**未**重新触发任何 Existing_Release_Tag（`v0.1.0` / `v0.1.0-rc1` / `v0.1.3` / `v0.1.3.1` / `v0.1.4` / `v0.1.5` / `v0.1.6` / `v0.1.6.1` / `v0.1.8`）上的 workflow（满足 Requirement 8.1 / 8.2 / 8.4）。同 PR #121 同时复用为 task 8.3（Pre-Merge Gate Checklist 6 项）/ 9.2（In_Scope_Fix 与 DOC shape 区分登记）/ 10.6（Property 6 — Polish-only diff shape 分类）的 audit 载体；`comment_url` 同时是本 GHA block 与「发现与处置 / Findings and Triage」表对应行的交叉引用锚点。四条 URL 已通过本地 regex / 一致性矩阵校验（pr_url ↔ comment_url `pull/121` 一致；pr_url ↔ run_url owner/repo `Hyperion-GPU/ProofFlow-v0.1` 一致；artifact_id 纯数字 `7063049346`），**不**对外网发请求（AGENTS.md `localhost as the default trust boundary`）。Audit trail：`.kiro/specs/v0.1.x-dogfood-and-channel-polish/transcripts/4_3_gha_channel.md`。

## 发现与处置 / Findings and Triage

> 由 Task 9.1 回填于 `2026-05-19T02:03:35+08:00`，登记 7 条 Dogfood_Finding；按 Requirement 7.2 严格判定，5 行 In_Scope_Fix 候选（F-1 ~ F-5）因对应 polish/quickstart-* PR 仍未合并，按 Requirement 7.5 backlog honesty 暂归 `Out_Of_Scope_Finding` 并挂新 anchor `PLANS.md#quickstart-polish-prs-pending-packaging-from-v0-1-x-dogfood`，待 5 个 PR merged 后由 task 9.2 升级回 `In_Scope_Fix` 并把 `link` 替换为真实 merged PR URL；F-6 / F-7 引用既有 deferred anchor（Channel Evidence VSCode block + Demo and Example Packet section 已交叉记录）。Audit trail：`.kiro/specs/v0.1.x-dogfood-and-channel-polish/transcripts/9_1_findings.md`。
...
| `proofflow-pr-review.yml` | <https://github.com/Hyperion-GPU/ProofFlow-v0.1/actions/runs/26045394262> | Pass | 与 task 4.3 PR #121 Run URL 交叉一致；workflow 由 `pull_request` 触发，无 main HEAD run；conclusion=success; head_sha=1ec968be34; created_at=2026-05-18T16:09:26Z |
...
> Pre-Merge Gate Checklist 表 + Deferred items 段由 Task 8.3 回填于 `2026-05-19T01:53:50+08:00`，对 6 个 Polish_PR 候选跑 §Pre-Merge Gate Checklist；本里程碑当前已开 PR 仅 1 条（#121，DOC shape，open 未合并），其余 5 条 polish/quickstart-* 仍在 working tree 待用户打包；audit trail：`.kiro/specs/v0.1.x-dogfood-and-channel-polish/transcripts/8_3_pre_merge_gate.md`。
...
| 每次 Polish_PR 合并后 | n/a | n/a | n/a (no Polish_PR merged this milestone; tasks 6.2 changes pending packaging into polish/* PRs by user) |
...
- 「每次 Polish_PR 合并后」一行写 `n/a`：本里程碑无 Polish_PR 合并到 `main`——task 6.2 的 5 份 Quickstart_Doc_Set 修复仍在 working tree（待用户后续打包 `polish/quickstart-*` 系列 PR 后由 task 8.3 补上 capture），task 4.3 PR #121 是专门为触发 GHA review 而提的纯文档 PR、与 design §GHA Channel Trigger Strategy 一致**未合并到 main**；按 design §Error Handling「把降级如实记录而不是粉饰为 Pass」原则，**不**伪造一行 capture 来填表。
...
| --- | --- | --- | --- | --- | --- | --- |
| <https://github.com/Hyperion-GPU/ProofFlow-v0.1/pull/121> (`polish/mcp-channel-dogfood`, head_sha `1ec968be34`, state OPEN) | Pass (task 8.2 capture-1/-2 + task 8.3 capture-3 三次 SHA-256 byte-identical: ls-remote `0B4AAEAC…329E`; gh release list `CACAB81B…979F`) | Pass (`gh pr diff 121 --name-only` 单文件 `docs/examples/proof_packet_v0_1_x_localproof_dogfood.md`，完全不在 `backend/proofflow/services/policy_gate_*.py` 或任何源码路径；Policy Gate scope 保持 `{move_file, rename_file}`) | Pass (task 2.2 transcript: rc_api_smoke / rc2_policy_gate_smoke / backup_restore_api_smoke / ci_agentguard_review.py --help 全 Pass；DOC-only diff 不触敏感文件 omission) | Pass (task 4.3 / 8.1 PR #121 Run URL <https://github.com/Hyperion-GPU/ProofFlow-v0.1/actions/runs/26045394262> conclusion=success；其余 5 个 workflow 在 main HEAD Pass，见 §CI 状态) | DOC (单文件 `docs/examples/proof_packet_v0_1_x_localproof_dogfood.md`，命中 §Schema 6 第 1 类) | n/a (PR open, not merged; `gh pr view 121` → state=OPEN, mergedAt=null, mergeCommit=null) |
| `polish/quickstart-readme` (pending PR by user; F-1) | n/a (PR not opened yet) | n/a (PR not opened yet) | n/a (PR not opened yet) | n/a (PR not opened yet) | DOC (planned by task 6.2) | n/a (PR not opened yet) |
| `polish/quickstart-readme-zhcn` (pending PR by user; F-2) | n/a (PR not opened yet) | n/a (PR not opened yet) | n/a (PR not opened yet) | n/a (PR not opened yet) | DOC (planned by task 6.2) | n/a (PR not opened yet) |
| `polish/quickstart-ledger-mcp` (pending PR by user; F-3) | n/a (PR not opened yet) | n/a (PR not opened yet) | n/a (PR not opened yet) | n/a (PR not opened yet) | DOC (planned by task 6.2) | n/a (PR not opened yet) |
| `polish/quickstart-codex` (pending PR by user; F-4) | n/a (PR not opened yet) | n/a (PR not opened yet) | n/a (PR not opened yet) | n/a (PR not opened yet) | DOC (planned by task 6.2) | n/a (PR not opened yet) |
| `polish/quickstart-v01-dogfood` (pending PR by user; F-5) | n/a (PR not opened yet) | n/a (PR not opened yet) | n/a (PR not opened yet) | n/a (PR not opened yet) | DOC (planned by task 6.2) | n/a (PR not opened yet) |

Pre-Merge Gate Checklist 落地说明：本表 6 行覆盖本里程碑的 6 个 Polish_PR 候选全集（task 4.3 PR #121 + task 6.2 working-tree-only 5 份 Quickstart_Doc_Set 修复对应的 5 个 polish/quickstart-* 分支）。PR #121 6/6 列均为 Pass / n/a（`merged-tag re-check` 一项因 PR 仍 OPEN 未合并而严格写 `n/a (PR open, not merged)`，按 design §Error Handling「把降级如实记录而不是粉饰为 Pass」原则不伪造结论）；5 个 polish/quickstart-* 行 6 列全部写 `n/a (PR not opened yet)`，待用户后续打包 PR 后由 task 8.3 增量回填实采。`Diff shape` 列对 5 行 placeholder 写 `DOC (planned by task 6.2)`：task 6.2 transcript §推荐分支命名 章节已显式声明每个分支只触及单份 Quickstart 文档（仅 `*.md` / `docs/`），与 §Schema 6 DOC shape 严格一致；PR 真实 open 后该列再切换为 `DOC` 实采（届时 `gh pr diff <pr> --name-only` 验证）。本表与 Existing_Release_Tag 集合骨架行 + 上方 Tag immutability snapshots 表完全独立——本任务**不**修改任何 tag 相关结构，仅复用 task 8.2 SHA-256 baseline 作为 capture-1 / capture-2 引用；本任务的即时 re-check capture-3 同样 byte-identical（详见 transcript `8_3_pre_merge_gate.md` § Step 1）。
...
- Policy Gate action 类型集合保持 `{move_file, rename_file}`，本里程碑无新增：与 design §Non-Goals 2 / Requirement 9.1 一致；6 个 Polish_PR 候选全集 Diff shape 均为 DOC（已 open 的 PR #121 + planned 5 个 polish/quickstart-*），trivially 不触及 Policy Gate 注册路径；行为级回归由 task 2.2 transcript `scripts/rc2_policy_gate_smoke.py` 14/14 Pass 持续守护。
...
- result:
  ...
  - 「发现与处置 / Findings and Triage」（task 9.1 落地）：F-1 ~ F-7 7 行 8 字段全填，`link` 列均为 PLANS anchor。
  ...
  - 「不变量保护与遗留」（task 8.2 / 8.3 落地）：3 次 capture SHA-256 byte-identical (`0B4AAEAC…329E` / `CACAB81B…979F`)；Pre-Merge Gate Checklist 表 PR #121 6 项实采 + 5 行 polish/quickstart-* placeholder 显式 `n/a (PR not opened yet)`（按 design §Error Handling 不伪造 Pass）。
...
- result:
  ...
  - **GHA** = Pass + 4 字段全在：`pr_url: https://github.com/Hyperion-GPU/ProofFlow-v0.1/pull/121` / `run_url: .../actions/runs/26045394262` / `artifact_id: 7063049346` / `comment_url: .../pull/121#issuecomment-4479556510`。
...
| --- | --- | --- | --- |
| `README.md` | `e3c6318ac159fe961dac42e9e0302325f8525ff0` | Pass | task 6.1 stale 5 → task 6.2 working tree 修复后 0；commit SHA 仍为 6.1 时刻值，待用户打包 `polish/quickstart-readme` PR 后回填新 SHA |
| `README.zh-CN.md` | `3bd818a6dcd3737f094b1dbee516c6abdd2bbe8f` | Pass | task 6.1 stale 6 → task 6.2 working tree 修复后 0；含 entry `app.main:app` → `proofflow.main:app` + 端口固定 `8787` + MCP 入口收敛到 `pip install proofflow-mcp`；commit SHA 仍为 6.1 时刻值，待用户打包 `polish/quickstart-readme-zhcn` PR 后回填新 SHA |
| `docs/ledger_quickstart_mcp.md` | `e3c6318ac159fe961dac42e9e0302325f8525ff0` | Pass | task 6.1 stale 2 → task 6.2 working tree 修复后 0；commit SHA 仍为 6.1 时刻值，待用户打包 `polish/quickstart-ledger-mcp` PR 后回填新 SHA |
| `docs/codex_workflow.md` | `29611c009cf1dbe080eb1a1a34da1ddde10e0f7d` | Pass | task 6.1 stale 3 → task 6.2 working tree 修复后 0；commit SHA 仍为 6.1 时刻值，待用户打包 `polish/quickstart-codex` PR 后回填新 SHA |
| `docs/V0_1_DOGFOOD.md` | `5f9e84d9919119a14586a0326f567a71187d54d9` | Pass | task 6.1 stale 4 → task 6.2 working tree 修复后 0；commit SHA 仍为 6.1 时刻值，待用户打包 `polish/quickstart-v01-dogfood` PR 后回填新 SHA |
...
- expected:
  ...
  - 任意 Polish_PR 合并 commit 上 `git tag --points-at HEAD` 为空集合或仅 `main`（本里程碑无 Polish_PR 合并，trivially 满足）。
- result:
  ...
  - 本里程碑无 Polish_PR 合并到 main（PR #121 仍 OPEN，5 个 polish/quickstart-* 未提），`git tag --points-at HEAD` n/a；trivially 满足 Tag immutability。
...
- check command:
  ...
  ```powershell
  Push-Location "<repo root>"
  # 1. 列出所有被合并到 main 的 Polish_PR；本里程碑应为空集
  gh pr list --state merged --base main --search "merged:>=2026-05-18" --json number,title,headRefName,mergedAt,url
  # 2. 对当前 OPEN 的 PR #121（GHA channel evidence trigger，非 In_Scope_Fix）跑 diff shape 分类
  gh pr diff 121 --name-only
  gh pr view 121 --json body | rg "Fixes finding F-[0-9]+|Resolves #[0-9]+"
  # 3. PR diff 黑名单关键词（任何被合并的 Polish_PR）
  gh pr diff 121 --name-only | rg "(cloud_sync|telemetry|webhook|multi_user|restore_to_live|apply_to_live_db)"
...
- result:
  - **本里程碑 In_Scope_Fix Polish_PR merged 集合 = 空集**（task 9.2 audit 确认）：5 个 polish/quickstart-* 未提（用户 Option B 决定不开 PR），PR #121 仍 OPEN。
  - **PR #121** diff = 单文件 `docs/examples/proof_packet_v0_1_x_localproof_dogfood.md` → DOC shape（task 8.3 step 2 + 5 已分类）；PR description 显式声明「专门为触发 GHA review 而提，非 In_Scope_Fix」（task 4.4 GHA block Notes L128 已记录）。OPPORTUNISTIC_FIX 不适用（DOC shape 不要求 `Fixes finding F-<n>` 锚点）。
  - **黑名单关键词**：PR #121 diff 0 命中（task 8.3 step 5 Diff shape 分类已确认非 OPPORTUNISTIC_FIX 路径）；本里程碑 0 OPPORTUNISTIC_FIX shape PR。
  - **Findings 表 `In_Scope_Fix` 行**：0 行（task 9.1 Option A 决策；7 行全部 `Out_Of_Scope_Finding`，task 9.2 §Step 1 实采 7 行 classification 列均为 `Out_Of_Scope_Finding`）。
  - Findings 表 In_Scope_Fix 集合（{}）与本里程碑被合并的 Polish_PR 集合（{}）一一对应（双方均为空集，trivially 一致）。
  - 5 个 polish/quickstart-* 未来打包 PR 时 planned shape = DOC（task 6.2 §推荐分支命名 + task 8.3 placeholder 行 5 cells `DOC (planned by task 6.2)`），由 task 9.2 增量回填路径在合并后升级到 In_Scope_Fix。
...
- result:
  ...
  - **task 9.3 §Step 1** 7/7 link 字段命中：F-1 ~ F-5 → `PLANS.md#quickstart-polish-prs-pending-packaging-from-v0-1-x-dogfood`；F-6 → `PLANS.md#vscode-channel-screenshots-deferred-from-v0-1-x-dogfood`；F-7 → `PLANS.md#agentguard-public-packet-deferred-from-v0-1-x-dogfood`。
  ...
  - 7 行 classification 列：0 `In_Scope_Fix` / 7 `Out_Of_Scope_Finding`（与 Property 6 In_Scope_Fix 集合 {} 一致）；按 Requirement 7.2 严格判定无静默丢弃，Backlog honesty 维持。
...
- result:
  - **task 8.1** 已在 Latest_Main HEAD `15b52e6741cd82efd6a1bb1ff6b8649b02b84d8b`（origin/main）采集 6 个 workflow Run URL，写入 Schema 5 表 6 行：
    ...
    - `proofflow-pr-review.yml` → runs/26045394262 (降级路径：复用 task 4.3 PR #121 Run URL；conclusion=success；与 GHA channel block 交叉一致)
  ...
  - 本里程碑无 Polish_PR merge 到 main（PR #121 仍 OPEN，5 个 polish/quickstart-* 未提），「每次 Polish_PR merge 后」时间点 trivially n/a；milestone 起点 = 终点（task 8.1 单次 capture），与 task 8.2 Tag immutability capture-1 / -2 byte-identical 形成同步证据。
...
- [x] DoD-4 (R4): 5 份 Quickstart_Doc_Set（`README.md` / `README.zh-CN.md` / `docs/ledger_quickstart_mcp.md` / `docs/codex_workflow.md` / `docs/V0_1_DOGFOOD.md`）在单一 PowerShell 会话验证完毕，各附 commit SHA + `actual_status`；ripgrep 三段 lint（`^cd ` / 非 `8787` 端口 / 含空格路径未引号）命中数 0/0/0；stale 步骤已在 working tree 修复待打包 5 个 polish/quickstart-* PR。详见 [Property 3: Quickstart reproducibility on a fresh Windows clone](#property-3-quickstart-reproducibility-on-a-fresh-windows-clone) 子表。
...
- [x] DoD-7 (R7): Findings 表 7 行（F-1 ~ F-7）8 字段全填，7/7 link 列命中合规——5 行 → `PLANS.md#quickstart-polish-prs-pending-packaging-from-v0-1-x-dogfood`、1 行 → `PLANS.md#vscode-channel-screenshots-deferred-from-v0-1-x-dogfood`、1 行 → `PLANS.md#agentguard-public-packet-deferred-from-v0-1-x-dogfood`，0 静默丢弃。详见 [#发现与处置 / Findings and Triage](#发现与处置--findings-and-triage) 与 [Property 7: Backlog honesty](#property-7-backlog-honesty)。
- [x] DoD-8 (R8): 本里程碑无新 release tag；milestone start / close / Pre-Merge Gate re-check 三次 capture `git ls-remote --tags origin` SHA-256 = `0B4AAEAC314AC6FF9B88B9EED2D3914479BB67F2B457F22DECB684CD4E88329E` 与 `gh release list` SHA-256 = `CACAB81B75FFBE57DD0B5BDB05019818E3EB8BADF4BA86B525876D65D754979F` byte-identical；Existing_Release_Tag 9 集合（`v0.1.0` / `v0.1.0-rc1` / `v0.1.3` / `v0.1.3.1` / `v0.1.4` / `v0.1.5` / `v0.1.6` / `v0.1.6.1` / `v0.1.8`）9/9 verified。详见 [#不变量保护与遗留 / Invariants and Deferred](#不变量保护与遗留--invariants-and-deferred) 与 [Property 4: Tag immutability](#property-4-tag-immutability)。
- [x] DoD-9 (R9): Pre-Merge Gate Checklist 6 项对 PR #121 6/6 Pass / n/a（`merged-tag re-check` 因 PR OPEN 未合并严格写 `n/a`，不伪造结论），其余 5 行 polish/quickstart-* 写 `n/a (PR not opened yet)`；Trust_Baseline 6 条不变量（action safety scope / hash-guarded undo / sensitive-file omission / Policy Gate fail-closed / live DB restore deferred / Existing_Release_Tag immutability）保持。详见 [#不变量保护与遗留 / Invariants and Deferred](#不变量保护与遗留--invariants-and-deferred) 与 [Property 5: Trust baseline preservation](#property-5-trust-baseline-preservation)。
...
- [x] DoD-12 (R12): 本里程碑 merged Polish_PR 集合 = {}（用户 Option B 未打包 5 个 polish/quickstart-* PR；PR #121 仍 OPEN 仅作 GHA channel evidence 触发载体）；Findings 表 `In_Scope_Fix` 行 = 0、`Out_Of_Scope_Finding` 行 = 7；0 OPPORTUNISTIC_FIX shape PR、0 net-new feature 关键词（`cloud_sync` / `telemetry` / `webhook` / `multi_user` / `restore_to_live` / `apply_to_live_db`）命中，trivially polish-only。详见 [Property 6: Polish-only diff shape](#property-6-polish-only-diff-shape) 与 [#发现与处置 / Findings and Triage](#发现与处置--findings-and-triage)。
...
- 用户后续按 PLANS anchor `quickstart-polish-prs-pending-packaging-from-v0-1-x-dogfood` §Required deliverables 把 5 份 Quickstart_Doc_Set working-tree 修复（README.md / README.zh-CN.md / docs/ledger_quickstart_mcp.md / docs/codex_workflow.md / docs/V0_1_DOGFOOD.md）打包为 5 个独立 `polish/quickstart-*` PR（DOC shape）并合入 `main`；每个 PR 通过后由后续里程碑 / task 9.2 把 Findings 表 F-1 ~ F-5 5 行从 `Out_Of_Scope_Finding` 升级回 `In_Scope_Fix`，并把 `link` 字段替换为真实 merged PR URL（`^https://github\.com/Hyperion-GPU/ProofFlow-v0.1/pull/[0-9]+$`）。
