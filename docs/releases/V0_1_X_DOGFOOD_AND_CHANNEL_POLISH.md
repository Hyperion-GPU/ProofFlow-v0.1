...
> 由 Task 8.1 回填于 `2026-05-19T01:35:56+08:00`，并由 PR #127 post-closure audit refresh 追加 `2026-05-19T06:05:06Z` 后的 main push runs；gh CLI 实跑（`gh --version` → 2.92.0；`gh auth status` → `Hyperion-GPU` keyring，Active account=true）取最新 `--branch main --limit 10` run；不可用时按 design §Risk 降级。Audit trail：`.kiro/specs/v0.1.x-dogfood-and-channel-polish/transcripts/8_1_ci_status.md`。
> 采集来源：`gh run list --workflow <w> --branch main --limit 10 --json databaseId,headSha,status,conclusion,url,createdAt,displayTitle`。
> 3 个无 path filter 的 push-on-main workflow（`backend.yml` / `frontend.yml` / `lint.yml`）在 closure PR #121 与 quickstart PR #122–#126 每次 squash merge 后均触发并 success；最终 post-closure main HEAD 为 `277fb0c1def1e95c12a3d2a1eaaf96fb6deddc6d`（PR #123）。`mcp-server.yml` 只在 `mcp-server/**` 路径变更时触发，PR #121–#126 均为 DOC-only diff，故其 main baseline 保持最近一次匹配触发的 success run `26042200116`（head_sha `15b52e6741...`），不伪造 post-closure main run。
...
> `gh` CLI 在本任务整体可用，无 GitHub UI 手动复制 Run URL 的降级路径；6 行表中 6/6 `Status = Pass`，其中 3 行为 post-closure final main HEAD success run，3 行为明示降级 / path-filter baseline。

| Workflow | Evidence Run URL | Status | Notes |
| --- | --- | --- | --- |
| `backend.yml` | <https://github.com/Hyperion-GPU/ProofFlow-v0.1/actions/runs/26079477454> | Pass | post-closure final main HEAD run; conclusion=success; head_sha=277fb0c1; created_at=2026-05-19T06:05:06Z |
| `frontend.yml` | <https://github.com/Hyperion-GPU/ProofFlow-v0.1/actions/runs/26079477433> | Pass | post-closure final main HEAD run; conclusion=success; head_sha=277fb0c1; created_at=2026-05-19T06:05:06Z |
| `lint.yml` | <https://github.com/Hyperion-GPU/ProofFlow-v0.1/actions/runs/26079477462> | Pass | post-closure final main HEAD run; conclusion=success; head_sha=277fb0c1; created_at=2026-05-19T06:05:06Z |
| `mcp-server.yml` | <https://github.com/Hyperion-GPU/ProofFlow-v0.1/actions/runs/26042200116> | Pass | path-filter baseline; workflow triggers only on `mcp-server/**`; PR #121–#126 are DOC-only and did not trigger a new main run; conclusion=success; head_sha=15b52e6741; created_at=2026-05-18T15:10:18Z |
...
- result:
  - **task 8.1 + post-closure refresh** 已在 final post-closure main HEAD `277fb0c1def1e95c12a3d2a1eaaf96fb6deddc6d`（PR #123 squash merge）采集 6 个 workflow evidence URL，写入 Schema 5 表 6 行：
    - `backend.yml` → runs/26079477454 (post-closure final main HEAD; conclusion=success)
    - `frontend.yml` → runs/26079477433 (post-closure final main HEAD; conclusion=success)
    - `lint.yml` → runs/26079477462 (post-closure final main HEAD; conclusion=success)
    - `mcp-server.yml` → runs/26042200116 (path-filter baseline; workflow triggers only on `mcp-server/**`; PR #121–#126 DOC-only diffs did not trigger a new main run; conclusion=success)
  ...
  - PR #121 与 quickstart PR #122–#126 merge 后的 push-on-main CI history 已复核：`backend.yml` / `frontend.yml` / `lint.yml` 在 6 个 squash commit（`a6874cb0` / `3a5195be` / `fdac327a` / `2575785b` / `7a49da4` / `277fb0c1`）上均有 success run；最终 head `277fb0c1` 的三条 Run URL 即为上表主证据。对应 run id 三元组（backend / frontend / lint）：#121 = `26076067208` / `26076067190` / `26076067200`；#124 = `26079392306` / `26079392305` / `26079392345`；#125 = `26079411286` / `26079411314` / `26079411288`；#126 = `26079429226` / `26079429168` / `26079429205`；#122 = `26079455320` / `26079455333` / `26079455373`；#123 = `26079477454` / `26079477433` / `26079477462`。`mcp-server.yml` 未在这些 DOC-only commits 上触发，符合 workflow path filter `mcp-server/**`。
  - GHA_Channel 主证据仍绑定 PR #121 的 Run `26074265258`，PR #127 自身 GHA Run `26080468655` / artifact `7076402293` / comment `#issuecomment-4485025048` 仅作为 PR body self-evidence，不写入 GHA_Channel 主字段以避免自指循环。
...
- [x] DoD-6 (R6): 6 个 workflow evidence URL 全部 `conclusion = success`，6/6 Run URL 命中正则 `^https://github\.com/Hyperion-GPU/ProofFlow-v0.1/actions/runs/[0-9]+$`；post-closure final main HEAD `277fb0c1…` 上 `backend.yml` / `frontend.yml` / `lint.yml` 三条 push-on-main run 绿，`mcp-server.yml` 因 path filter `mcp-server/**` 未被 DOC-only PR #121–#126 触发而保留最近匹配 baseline，`proofflow-pr-review.yml` 复用 PR #121 Run（与 GHA channel block 交叉一致），`publish-mcp.yml` 取 `mcp-v0.1.2` baseline。详见 [#CI 状态 / CI Status](#ci-状态--ci-status) 与 [Property 8: CI green maintenance across the milestone](#property-8-ci-green-maintenance-across-the-milestone)。
