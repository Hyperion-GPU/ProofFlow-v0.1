...
def test_design_doc_marks_phase2_and_phase3_backend_endpoints_implemented():
    content = _doc()
    assert "Phase 2 implements the backup endpoints" in content
    assert "Phase 3 implements restore preview and restore-to-new-location" in content
    assert "Status: implemented in Phase 3." in content
    assert "Live DB restore remains blocked." in content
